...
GitHub Actions checks naming, formatting, and API documentation. CircleCI also compiles the submitted project's Game, Editor, and Server targets with Unreal Engine 5.6.1 on Linux before merge, whether the PR comes from a fork or a branch in this repository. You do not need a CircleCI account or any additional setup. If a fork PR changes CI workflows, automation, Unreal build scripts, or plugin descriptors, discuss it with a maintainer; these changes need a branch in the source repository. See the [development and CI workflow](Docs/DevelopmentWorkflow.md) for the check sequence and build scope. Independent framework capabilities require automated tests; CI does not enforce a numeric code coverage threshold.
...
## Restricted Git LFS contributions

This project uses a restricted Git LFS workflow. LFS objects are stored in a separate GitLab project, so permission to push to a GitHub fork does not grant permission to upload LFS objects. Before adding or changing an LFS-tracked file, including a binary library, or changing which files use LFS, contact a maintainer through an issue to agree on the contribution and arrange the upload. Pull requests that introduce new or changed LFS objects without prior coordination will be rejected. Contributions that do not change LFS files can follow the normal fork and pull request process.
