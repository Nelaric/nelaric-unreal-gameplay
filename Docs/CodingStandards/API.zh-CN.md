...
审查者需检查第一个 `public:` 区域中的方法是否有有效的 Doxygen 注释，并确认两段之间没有其他访问说明符。PR 检查会生成 Doxygen HTML，出现文档警告即失败。提交不写注释的框架内部方法前，须先让文档工具与这一区分保持一致。合并后，已通过检查的文档会发布到 GitHub Pages。
