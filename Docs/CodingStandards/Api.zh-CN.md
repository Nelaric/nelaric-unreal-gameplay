...
- 稳定的公开接口使用 UE 类型。Provider 契约使用纯 C++ 接口；只有需要面向 Blueprint 时才另设适配层。
...
所有公开类型、方法、枚举及枚举值、常量、Provider 契约，以及向 Blueprint 暴露的入口，都必须有英文 Doxygen 注释。注释应解释用途和用法，并在适用时说明参数与返回值、所有权与生命周期、线程要求，以及失败、超时和取消语义。每个公开头文件必须包含 `@file` 或 `\file` 注释，以便文件级声明进入生成文档。

CI 会生成 Doxygen HTML，并在出现警告或未记录的公开声明时失败。审查者还会检查注释是否准确解释行为；仅有形式上的注释并不够。变更进入 `main` 后，API 文档会发布到 GitHub Pages。
