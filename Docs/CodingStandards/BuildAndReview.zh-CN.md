...
- 面向 UE 5.6 及以上版本。引入引擎编译 CI 后，应同时覆盖最低支持版本和最新支持版本。现阶段不要求 UE 编译任务或 Runner。
...
审查时应关注正确性、版权声明、模块边界、公开 API 文档、取消与失败行为、所有权、性能和密钥处理。申请例外的 PR 必须说明规则、原因、受影响代码和替代方案，并获得维护者批准。当前 CI 通过即可满足自动检查门槛；若存在具体行为风险，仍可添加测试。
