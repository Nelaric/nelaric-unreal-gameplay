...
请求审查前，请先运行与改动相关的本地检查。PR 必须通过格式、API 文档、PR 命名和 Linux 插件编译状态。对于来自 Fork 的 PR，CircleCI 会在合并前编译提交的插件代码。修改 CI 工作流、自动化脚本、Unreal 构建脚本或插件描述文件时，请由维护者在源仓库分支处理。合并到 `main` 后，CI 会再次编译插件。现阶段不要求 clang-tidy 或测试覆盖率门槛。
