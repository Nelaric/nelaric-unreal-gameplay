...
- **必须**为所有公开类型、方法、枚举及枚举值、常量、玩法扩展契约和 Blueprint 入口编写文档注释。公开属性与字段（包括反射属性）也必须有注释。派生类型依赖的受保护声明应按其契约编写注释。
...
CI 检查注释覆盖、Doxygen 警告和机械格式。审查者需判断英文注释是否准确描述代码契约，包括相关的所有权与运行时行为。运行改动所需的[文档与文本检查](BuildAndReview.zh-CN.md#本地检查)。Doxygen 生成过程没有警告，不能代替内容审查。
