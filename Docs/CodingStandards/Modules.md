...
- Core modules must not depend on a vendor SDK, online backend, content-distribution provider, or optional gameplay integration. Optional modules may depend on Core. Circular module dependencies are forbidden.
...
- Keep Core genre-independent. Common match flow, objectives, scoring, and player lifecycle belong in Core. Concrete combat, inventory, quests, and character rules belong to the consuming game or optional extensions.
- Use UE networking for gameplay state replication. Core must not implement a second transport or require a backend for standalone play.
- Keep content-version and update-delivery mechanisms separate from match rules. Core may consume a validated content version without depending on a particular patching service.
