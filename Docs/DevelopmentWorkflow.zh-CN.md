...
2. 向 `main` 发起 PR。GitHub Actions 的 **Quality** 工作流会执行质量检查。另一条可信的 GitHub Actions 工作流会核验 PR 目标、准确的提交 SHA 和改动文件；符合条件的 Fork PR 会启动 CircleCI GitHub App Linux 编译，并将结果回报到该 PR 提交。合并前必须通过必需检查并完成审查。
...
GitHub Actions 的 **Quality** 工作流会在 Fork PR 上运行，不使用项目密钥。三项必需检查覆盖格式与文本规范、公开 API 文档以及 PR 和提交命名。贡献者不需要创建 CircleCI 项目或获取项目凭据。

`Protect main` 规则集要求这三项 GitHub Actions 质量检查以及由 GitHub Actions 上报的 `ci/fork-pr-linux-build`。

## 合并前的 Linux 编译

`Fork PR Linux build` GitHub Actions 工作流通过 `pull_request_target` 在可信的目标分支版本上运行，绝不在 GitHub Runner 上检出或执行 PR 文件。对于 Fork PR，它确认打开的 PR 指向 `Nelaric/nelaric-unreal-server:main`，固定当前 head SHA，并拒绝修改 `.circleci/`、GitHub Actions 工作流与自动化文件、`Scripts/`、Unreal 构建脚本、插件描述文件、`.gitattributes` 和 `.gitmodules`。如需修改这些受保护文件，请由维护者在源仓库分支处理；该分支会收到表示无需 Fork 编译的成功状态。
...
Custom Webhook URL 已存为 GitHub 仓库 Secret `CIRCLECI_PR_BUILD_WEBHOOK_URL`。CircleCI GitHub App 触发器的配置和检出分支均为 `main`。GitHub 工作流仅需 `contents: read`、`pull-requests: read` 和 `statuses: write`，无需单独的 bot 账号。Fork PR #21 已验证编译与 GitHub 状态回报，随后 `ci/fork-pr-linux-build` 被设为 `main` 的必需状态，预期来源是 GitHub Actions。CircleCI 项目的流水线信息公开，bot 通过其 API 查询对应作业的结果。
...
CircleCI 项目通过环境变量保存 `GHCR_USERNAME` 和 `GHCR_TOKEN`。`GHCR_USERNAME` 是有权拉取 Epic 引擎镜像的 GitHub 账号；`GHCR_TOKEN` 是仅授予 `read:packages` 权限的 classic 个人访问令牌。不要把令牌写入仓库、PR 或日志。可信的 PR 编译作业只能用这些变量拉取镜像，绝不能把它们传给隔离的插件编译容器。令牌过期或被撤销后，应在 CircleCI 中替换。
