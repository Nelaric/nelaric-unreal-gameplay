...
一次 CircleCI 流水线会同时启动三个独立的 Linux Job，使用 Unreal Engine 5.6.1 构建 `NelaricGameplay/NelaricGameplay.uproject`，分别以 Development 配置编译 `NelaricGameplay`（Game）、`NelaricGameplayEditor`（Editor）和 `NelaricGameplayServer`（Server）Target。每个 Job 都会检出同一 PR 提交并使用独立工作区，避免 Unreal 构建产物冲突。启用的 `NelaricGameplayFoundation` 插件会随项目一起编译。只有三个 Job 都成功，工作流才会成功。工作流不发布可下载的打包产物；每个 Job 都会重新下载引擎镜像，且不使用编译缓存。
